...
if [ -n "$EXAMPLE_APPNAME_ENABLED" ]; then
  if [ "$EXAMPLE_APPNAME_ENABLED" != "yes" ]; then
...
# root user name
root_user_name="${EXAMPLE_ROOT_USER_NAME:-}"
# root user password
root_user_pass="${EXAMPLE_ROOT_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Normal user info [password/random]
# normal user name
user_name="${EXAMPLE_USER_NAME:-}"
# normal user password
user_pass="${EXAMPLE_USER_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Load variables from config
# Generated by my dockermgr script
if [ -f "/config/env/example.script.sh" ]; then
  . "/config/env/example.script.sh"
fi
# Overwrite the variables
if [ -f "/config/env/example.sh" ]; then
...
__update_conf_files() {
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # replace variables
  # __replace "" "" "$CONF_DIR/example.conf"
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$EXAMPLE_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$EXAMPLE_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$EXAMPLE_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$EXAMPLE_USER_PASS_WORD}"             #
